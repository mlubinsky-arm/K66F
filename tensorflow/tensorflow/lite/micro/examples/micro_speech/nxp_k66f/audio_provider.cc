...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter *error_reporter,
  ...
  //printf("\n GetAudioSamples START");
  //return kTfLiteOk; //MICHAEL



  if (!g_is_audio_initialized) {
     //printf("\n GetAudioSamples  before  InitAudioRecording");
    TfLiteStatus init_status = InitAudioRecording(error_reporter);
     //printf("\n GetAudioSamples  after  InitAudioRecording");
    if (init_status != kTfLiteOk) {
      //printf("\n GetAudioSamples  not initialized");
      return init_status;
    }
    g_is_audio_initialized = true;
  }
  //printf("\n GetAudioSamples  before start_offset");
  ...
  //printf("\n GetAudioSamples 1    duration_sample_count=%d", duration_sample_count);
  for (int i = 0; i < duration_sample_count; ++i) {
    // printf("\n GetAudioSamples i=%d    ", i);
    const int capture_index = (start_offset + i) % kAudioCaptureBufferSize;
    //printf("\n GetAudioSamples capture_index=%d    ", capture_index);
...
int32_t LatestAudioTimestamp() { 
  //printf("\n INSIDE LatestAudioTimestamp --- \n");
  //printf ("\n g_latest_audio_timestamp=%d", g_latest_audio_timestamp);
